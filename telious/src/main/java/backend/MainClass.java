...
public class MainClass {
	...
	public ArrayList getData() {
		...
		ResultSet rs = null;
		try {
			String sql="SELECT S.CUSTNAME,S.PHONENUMBER,S.ENTDATE,I.ITEMNAME,I.MFGDATE,I.EXPDATE,S.NOS,P.PAYNAME FROM SUPERMARKET5 S,ITEMS5 I,PAYMENT5 P  WHERE S.ITEM=I.ITEMID AND S.PAYMENT=P.PAYID";
	...
	public Integer save(String custName, String phoneNo, String entDate, String item, String nos, String pay) {
		int retString = 0;
		String sql = "INSERT INTO SUPERMARKET5 (CUSTNAME,PHONENUMBER,ENTDATE,ITEM,NOS,PAYMENT) VALUES (?,?,?,?,?,?)";
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public ArrayList getComboVals() {
		...
		try {
			con = jdbc.getConnection();
			ps = con.prepareStatement("SELECT ITEMID,ITEMNAME FROM ITEMS5");
			rs = ps.executeQuery();
			while (rs.next()) {
				id = rs.getString(1);
				name = rs.getString(2);
			...
			ps.close();
			rs.close();
			ps = con.prepareStatement("SELECT PAYID,PAYNAME FROM PAYMENT5");
			rs = ps.executeQuery();
			while (rs.next()) {
				id = rs.getString(1);
				name = rs.getString(2);
